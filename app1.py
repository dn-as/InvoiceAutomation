...
def check_taxinfo(po_mock,invoice_data):
    
    gl_entity = po_mock[0]['gl_entity_id'].lower()
    
    
    job_id = po_mock[0]['job_id'].upper()
    job_id = job_id[:2]
    glaccountvalue=''
    print('JOBBBBBBBB ID:' )
...
def can_close_po(invoice_items, po_items):
    ...
    for po in po_items:
        item_id = po['item_id'].lower()
        ordered = po['ordered']
        received = po['received']
        vouchered = po['vouchered']
...
def validatevouch(invoice_items, po_items):
    ...
    for item in invoice_items:
        part_number = item['SellerPartNumber'].lower()
        matching_pos += [po for po in po_items if po['item_id'].lower() == part_number]
        
    isVoucherEligible = True;     
        
    for match in matching_pos:
        part_number_po=match['item_id']
        ordered = match['ordered']
        received = match['received']
        vouchered = match['vouchered']
...
def validate_invoice_items_against_po(invoice_items, po_items):
    ...
    po_item_ids = {item['item_id'].lower() for item in po_items}
...
def validate_single_po(invoiceID,invoice_data):
    # Find invoice by ID
    ...
    purchase_order = next((po for po in po_mock if po['po_ID'].lower() == invoice_po_number), None)
    
    if not purchase_order:
        return {'status': 'error', 'message': f'purchase order id  {invoice_po_number} not found.'}, 404
    print("FOUND PO FOR THIS INVOICE",purchase_order)
...
def get_po_data(invoiceID):
    ...
    print(invoice_data)
    result = validate_single_po(invoiceID,invoice_data)
    
    #Validate Invoice against PO if it passes calculate tax etc.
    if result is not True:
        return jsonify(result[0]), result[1]

    if not validate_invoice_items_against_po(invoice_data, po_mock):
    ...
    if not validatevouch(invoice_data, po_mock):
        return jsonify({'status': 'error', 'message': 'Vouchering failed'}), 400
    
    
    close_po=can_close_po(invoice_data, po_mock)
    ...
    if IsInlineTax=='yes':
        hastax=True
        tax_info=check_taxinfo(po_mock,invoice_data)
   ...
   #get gl_entity by glentty or by job_id
    ...
    if  gl_entity:
        short_gl = po_mock[0]['gl_entity_id'].lower()
